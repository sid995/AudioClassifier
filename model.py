...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, fmap_dict=None, prefix=""):
        ...
        if fmap_dict is not None:
            fmap_dict[f"{prefix}.relu"] = out_add
...
class AudioCNN(nn.Module):
    ...
    def forward(self, x, return_feature_maps=False):
        if not return_feature_maps:
            ...
            for ind, block in enumerate(self.layer1):
                x = block(x, feature_maps, prefix=f"layer1.block{ind}")
            feature_maps["layer1"] = x

            for ind, block in enumerate(self.layer2):
                x = block(x, feature_maps, prefix=f"layer2.block{ind}")
            feature_maps["layer2"] = x

            for ind, block in enumerate(self.layer3):
                x = block(x, feature_maps, prefix=f"layer3.block{ind}")
            feature_maps["layer3"] = x

            for ind, block in enumerate(self.layer4):
                x = block(x, feature_maps, prefix=f"layer4.block{ind}")
